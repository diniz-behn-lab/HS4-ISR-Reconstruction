...
script_directory <- "Method_functions"
...
# participant<- 2
df <- pull_complete_cpep_data("Z:/LEADStudentsTrainees/Ramirez, Daniel/120034/HS4 Project/Derived Datasets/Clean_HS4_Data_For_ISR.xlsx")
...
legend("topright",legend=c(expression(bold("95% Credible Bounds")), expression(bold("Estimated ISR"))),
       lty=c(1,1),lwd = 2,col=c('#CFDCE9',"#21314d"),pch = c(15,19),
       cex = 1,pt.cex =1)
...
legend("topright" ,legend=c("True C-Pep", "Approx C-pep"),
       lty=1:2,lwd = 2,col=c("black","red"),pch = c(19,1),cex = 0.7)
...
result <- list(partic = df$ID[participant], bmi_cat = df$Group_BMI[participant],insulin0 = df$Insulin0min[participant],
               ...
               lowbound30AUC = lb30m, upbound30AUC = ub30m, lowbound3hAUC = lb3h, upbound3hAUC = ub3h,mae = MAE)
